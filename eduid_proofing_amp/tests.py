...
class AttributeFetcherNewToNewUsersTests(MongoTestCase):
    ...
    def setUp(self):
        ...
        super(AttributeFetcherNewToNewUsersTests, self).setUp(celery, get_attribute_manager, am_settings=am_settings)
        self.plugin_contexts = [
            oidc_plugin_init(celery.conf),
            letter_plugin_init(celery.conf)
        ]
    ...
    def tearDown(self):
        for context in self.plugin_contexts:
            context.userdb._drop_whole_collection()
        super(AttributeFetcherNewToNewUsersTests, self).tearDown()
